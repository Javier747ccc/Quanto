...
void Quantum::executeCU1(string angle, string s)
{
	...
	try
	{
		if (v.size() > 1)
		{
			if (qregs[v[0]].getBlochSpherePoint().getZ() > 0)
			{
...
void Pruebas()
{
	...
	double W = 3.86E26;
	double radioATierra = 1.5E11;
	double radioSol = 7E8;
	double temperaturaSol = 5504;

	double SuperficieRadioSol = 4 * pi * radioSol * radioSol;
	double SuperficieRadioATierra = 4 * pi * radioATierra * radioATierra;

	double ESol = W / SuperficieRadioSol;
	double ETierra = ESol * (SuperficieRadioSol / SuperficieRadioATierra);
	double temperaturaTierra = temperaturaSol * (SuperficieRadioSol / SuperficieRadioATierra);

	double porc = SuperficieRadioATierra / SuperficieRadioSol;

	double p = (ETierra * radioATierra * radioATierra) / (5.67E-8 * radioATierra * radioATierra);
	double tempTierra = sqrt(sqrt(p));

	/*
	double Esol = W / (4 * pi * radioASol * Rsol);
	double Etierra = Esol * (Rsol / R) * (Rsol / R);
	*/
    smatch match;
    string str = "q[1],q[0]";
